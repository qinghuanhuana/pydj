...
def add_guest(request):
    ...
    if len(guest_limit) >= event_limit:
        return JsonResponse({'status': 10024, 'message': 'event number is null'})
...
def get_guest_list(request):
    ...
    if eid == '':
        return  JsonResponse({'status': 10021, 'message': 'eid empty'})
...
def user_sign(request):
    ...
    result = Event.objects.get(id=eid).status
    if not result:
        return JsonResponse({'status':10023, 'message': 'not available'})
    ...
    if n_time >= e_time:
        return JsonResponse({'status':10024, 'message': 'has started'})

    result = Guest.objects.filter(phone=phone)
    if not result:
        return JsonResponse({'status':10025, 'message': 'phone null'})

    result = Guest.objects.filter(event_id=eid,phone=phone)
    if not result:
        return JsonResponse({'status':10026, 'message': 'not participate'})

    result = Guest.objects.get(event_id=eid,phone=phone).sign
    if result:
        return JsonResponse({'status': 10027, 'message': 'has sign in'})
    else:
        Guest.objects.filter(event_id=eid,phone=phone).update(sign='1')
        return JsonResponse({'statu': 200, 'message': 'sign success'})